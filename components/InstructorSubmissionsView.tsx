...
export default function InstructorSubmissionsView({ classroomId }: { classroomId: string }) {
  ...
  const fetchData = async (url: string, key: 'assignments' | 'submissions') => {
    try {
      ...
      console.log(`Fetched ${key}:`, data); // Log the API response
  ...
  useEffect(() => {
    setState(prev => ({ ...prev, submissions: [], selectedStudentId: null, error: '' })); // Clear old submissions
    if (state.selectedAssignmentId !== null) {
      fetchData(`/api/student_submissions?assignmentId=${state.selectedAssignmentId}`, 'submissions');
    }
  ...
  const resetStudentSelection = () => {
    setState(prev => ({ ...prev, selectedStudentId: null }));
  };
      ...
      {state.selectedAssignmentId && (
        <>
          <StudentList
            ...
            onSelectStudent={id => {
              setState(prev => ({ ...prev, selectedStudentId: id }));
            }}
            onBack={resetStudentSelection} // Pass the reset function as a prop
          />
